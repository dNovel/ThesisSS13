...
\begin{lstlisting}[caption=LINQ Query - Finde Index einer Kante falls vorhanden.]
...
Durch die Gegebenheiten der Implementierung von List<T> (Generic Lists) in \CSS ist es leider nicht m"oglich, die Container bei "Anderungen direkt in einer Liste zu manipulieren. Jeder Container muss erst aus der Liste kopiert werden um ihn dann zu "andern und anschliessen an der gleichen Stelle wieder einzuf"ugen. N"otig hierzu ist, dass der Index des Containers in der jeweiligen Liste bekannt ist. Hierzu ein Beispiel welches die Erkl"arung etwas erl"autern soll.
\lstinputlisting
...
\begin{lstlisting}[caption=Lambda Ausdruck - Aufschl"usselung.]
...
\begin{lstlisting}[caption=Gleiche Funktionalitat formuliert als LINQ Abfrage in Methodensyntax., label={code:alg3}]
...
\begin{lstlisting}[caption=Ausdruck formuliert in Abfragesyntax.,label={code:alg2}]
...
%Nativ
\chapter{Messergebnisse und Erkenntnisse}
...
\captionof{figure}{Messung des Enumerators EnVertexIncomingHalfEdge() im Gesamtkontext einer Demo Anwendung ohne LINQ. Die relevante Zeile ist blau markiert.}
\label{messung:alg11}
\end{center}
%Nativ
%Methodensyntax
\subsection{Messergebnis f"ur die Methodensyntax aus (alg2) \ref{messung:alg20}}
...
\captionof{figure}{Messung des Enumerators EnVertexIncomingHalfEdge() in Methodensyntax.}
...
\captionof{figure}{Messung des Enumerators EnVertexIncomingHalfEdge() in Methodensyntax im Kontext eines Demo Ablaufs.}
\label{messung:alg21}
\end{center}
...
%Methodensyntax
%Abfragesyntax
\subsection{Messergebnis f"ur die Abfragesyntax (alg3) aus \ref{code:alg3}}
...
\captionof{figure}{Messung des Enumerators EnVertexIncomingHalfEdge() in Abfragesyntax.}
\label{messung:alg31}
\end{center}
...
\captionof{figure}{Messung des Enumerators EnVertexIncomingHalfEdge() in Abfragesyntax im Kontext eines Demo Ablaufs.}
\label{messung:alg32}
\end{center}
%Abfragesyntax
...
\newpage
\section {Wo ist der Sourcecode und die Dokumentation zu \LFGS zu beziehen?}
	%Content
